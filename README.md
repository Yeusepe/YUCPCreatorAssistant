...
- External integration contract gate: `bun run test:external-integrations`. This is the fast PR-facing slice for provider/runtime/API/consumer hardening. It keeps provider runtime contracts in `packages/providers/test/*/module.test.ts`, API ownership and actor-scoping guards in `apps/api/src/routes/*.test.ts`, RPC normalization in `apps/api/src/internalRpc/router.test.ts`, and degraded-state consumer copy/CTA coverage in `apps/bot/test/lib/setupCatalog.test.ts` plus the targeted web dashboard/account tests.
