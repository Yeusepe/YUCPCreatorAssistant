...
- To have the dev supervisor launch `cdngine`, set `CDNGINE_DIR` in `.env.local`, `.env.infisical`, or your shell env to the checkout you want to run, for example `CDNGINE_DIR=C:\\Users\\svalp\\OneDrive\\Documents\\Development\\antiwork\\cdngine`. The default startup mode is `CDNGINE_START_MODE=server`, which starts the local platform and then runs the long-lived demo API server without the demo frontend. Use `CDNGINE_START_MODE=demo` only when you explicitly want the full demo UI as well, or `CDNGINE_START_MODE=stack` for a bootstrap-only Docker bring-up. Both `bun run dev` and `bun run dev:infisical` print whether `cdngine` is enabled and which mode it will use, and if `CDNGINE_DIR` is set to a bad path they fail immediately instead of silently skipping `cdngine`.
