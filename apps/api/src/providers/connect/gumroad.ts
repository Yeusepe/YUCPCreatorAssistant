...
import { PURPOSES as GUMROAD } from '../gumroad';
...
async function gumroadCallback(request: Request, ctx: ConnectContext): Promise<Response> {
  ...
  try {
    ...
    const accessEncrypted = await encrypt(accessToken, config.encryptionSecret, GUMROAD.credential);
    const refreshEncrypted = refreshToken
      ? await encrypt(refreshToken, config.encryptionSecret, GUMROAD.refreshToken)
      : undefined;
...
const gumroadConnect: ConnectPlugin = {
  providerId: 'gumroad',
  routes,
};

export default gumroadConnect;
