...
describe('POST /api/connect/user/verification/:intentId/manual-license', () => {
  beforeEach(() => {
  ...
  it('routes hosted manual-license verification through the public Convex action instead of API-side internal mutations', async () => {
    ...
    const response = await routes.postUserVerificationManualLicense(
      new Request('http://localhost:3001/api/connect/user/verification/intent_123/manual-license', {
        method: 'POST',
    ...
    expect(convexActionMock).toHaveBeenCalledWith(
      'verificationIntents.verifyIntentWithManualLicense',
      {
