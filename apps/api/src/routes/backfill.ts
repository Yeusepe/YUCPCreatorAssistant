  ...
  async fetchPage(
    ...
    while (true) {
      try {
        ...
        const facts: BackfillRecord[] = await Promise.all(
          orders
            .filter((o) => o.product_id === productRef)
            .map(async (order) => {
              ...
              return {
                ...
                lifecycleStatus: (order.status === 'completed' ? 'active' : 'refunded') as
                  | 'active'
                  | 'refunded'
                  | 'disputed',
                purchasedAt: order.created_at ? new Date(order.created_at).getTime() : Date.now(),
              };
            })
        );
        ...
        // Jinxxy uses X-RateLimit-Reset (not Retry-After), but error object may not carry it.
        // Re-throw non-rate-limit errors immediately.
        if (
          err instanceof Error &&
          (err.message.includes('429') || err.message.toLowerCase().includes('rate limit'))
        ) {
          const waitMs = 60_000; // conservative 1-minute wait
...
class LemonSqueezyBackfillAdapter implements BackfillProviderAdapter {
  ...
  async getCredential(
    ...
    if (!conn?.lemonsqueezyApiKeyEncrypted) return null;
    return decrypt(conn.lemonsqueezyApiKeyEncrypted, encryptionSecret);
  }
...
async function runBackfill(
  ...
  while (true) {
    ...
    if (facts.length > 0) {
      ...
      logger.debug('Backfill batch ingested', {
        provider,
        productId,
        providerProductRef,
