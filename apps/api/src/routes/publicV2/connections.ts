...
export async function handleConnectionsRoutes(
  ...
  if (subPath === '/connections') {
    ...
    try {
      const result = await convex.query(api.providerConnections.listPublic, {
      ...
      logger.error('providerConnections.listPublic failed', { error: String(err) });
      return errorResponse('internal_error', 'An internal error occurred', 500, reqId);
    }
  }

  const idMatch = subPath.match(/^\/connections\/([^/]+)$/);
  if (idMatch) {
    if (request.method !== 'GET') {
      return errorResponse('method_not_allowed', 'Method not allowed', 405, reqId);
    }
    ...
    try {
      const result = await convex.query(api.providerConnections.getConnectionPublic, {
        apiSecret: config.convexApiSecret,
        authUserId: auth.authUserId,
        connectionId,
      });
      ...
      logger.error('providerConnections.getConnectionPublic failed', { error: String(err) });
      return errorResponse('internal_error', 'An internal error occurred', 500, reqId);
    }
  }
